...
const CardHeaderComponent: React.FC<CardHeaderProps> = ({ title, subtitle, action, children, className = '' }) => (
  <div className={clsx('flex items-start justify-between mb-4 pb-4 border-b border-slate-200 dark:border-slate-600', className)}>
    {children ? (
      children
    ) : (
      <div>
...
const CardFooterComponent: React.FC<CardFooterProps> = ({ children, className = '' }) => (
  <div className={clsx('mt-4 pt-4 border-t border-slate-200 dark:border-slate-600', className)}>
    {children}
  </div>
)
